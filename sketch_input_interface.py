...
def converter():
    ...
    key_id = str(time_stamp[0:4]) + str(time_stamp[5:7]) + str(time_stamp[8:10]) + str(time_stamp[11:13]) + str(
        time_stamp[14:16]) + str(time_stamp[17:19]) + "00"
    ...
    # Furkan's code: Interface purposes
    with open('./interface_ndjson_files/new_drawing.ndjson', 'w') as f:
        writer = ndjson.writer(f, ensure_ascii=False)
        writer.writerow(drawing_map)
